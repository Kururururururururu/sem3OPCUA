  ...
  brew: async (beer_type, beer_amount, machine_speed) => {
    try {
      ...
      if (machine_speed < 0 || machine_speed > speedLimits[beer_type]) {
        console.error(
          `Invalid machine speed for ${beer_type}. It should be between 0 and ${speedLimits[beer_type]}.`
        );
        return false;
      }
      ...
      console.log(
        `Brewing ${beer_amount} of beer type ${beer_type} at a speed of ${machine_speed} beers per minute`
      );
      return true;
    } catch (err) {
